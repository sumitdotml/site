...
<p>
  What is the attention weight trying to tell us? <br />
  <a href="#intuition-on-attention-weights">I know now how to think about it</a>.

  There are batches of tokens that are being processed in parallel to get the output (input + output_proj, i.e., the updated "residual stream") from the MHA sub-layer of the transformer. How do I exactly visualize this? What happens sequentially? What happens in parallel? <br />
  Need to get my head around this. I'm sure this is much simpler than I'm making it out to be. Will update this soon.

  Why do we need the output projection? What is the intuition behind it? <br />
  This is basically how much to move the information forward based on the attention weights. Will update more details on this to come.
</p>
