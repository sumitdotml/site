...
These calculations (token ID calculation, tokenization + vocab prep, embedding, positional encoding) are not done more than once. After the resulting embedded + positionally encoded input IDs get fed into the first `sublayer`, it is, as the paper<FootnoteRef id="1" /> I'm reading claims, a residual stream that flows sequentially across each layer of the transformer (layer = transformer block).
